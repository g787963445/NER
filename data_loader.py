...
from getweibozixingid import zixingtoid,wordtozixing,produce_zixing_id_mask
...
def sequence_padding_for_bert(X, y, Z, tokenizer, labels, max_len):
    ...
    for i, sequence in tqdm(enumerate(X)):
        ...
        zixing_ids = wordtransid(sequence)
        zixing_ids = [[cls_id]*16] + zixing_ids[:max_len - 2] + [[sep_id]*16] + [[pad_id]*16]* (max_len - len(zixing_ids) - 2)
        ...
        input_labels = [sequence_labels.pop() if pred_mask[i] == 1 else labels.index("O") for i in
                        range(len(pred_mask))]
...
class NERDataset(Dataset):
    ...
    def __init__(self, file_path, labels, word2id=None, tokenizer=None, max_len=128, is_BERT=True):
        ...
        if is_BERT:
            self.input_ids, self.attention_masks, self.pred_mask, self.input_labels, self.zixing_ids = sequence_padding_for_bert(
                self.X, self.y, self.Z, tokenizer, labels, max_len=max_len)
        # else:
